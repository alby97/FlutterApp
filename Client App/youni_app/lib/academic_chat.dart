...
class AcademicChat extends StatefulWidget {
  final String chatName;
  final String id;

  AcademicChat({this.chatName, this.id});

  AcademicChatState createState() =>
      AcademicChatState(chatName: chatName, id: id);
}

class AcademicChatState extends State<AcademicChat>
  ...
  AcademicChatState({this.chatName, this.id});
  ...
  void initState() {
    ...
    getValue("Username").then((value) {
      ...
      SharedPreferences.getInstance().then((prefs) {
        ...
        joined = chatsJoined.contains("chat_acc_"+id);
        if (joined == true) {
          initSocket();
        }
        getToken().then((token) {
          http.post(getUrlHome() + "getAcademicMessages",
  ...
  void initSocket() async {
    ...
    socket.onConnect((data) {
      ...
      if (chatsJoined != null /*&& !chatsJoined.contains(chatName)*/) {
        var j = chatsJoined.contains("chat_acc_"+id);
        socket.emit("join", [
          {
            "chatName": chatName,
            "username": myUsername,
            "joined": j,
            "idChat": id,
            "is_academic": true.toString()
          }
        ]);
        if (j == false) {
          // subscribeChat(chatName);
          subscribeChatNew(id, true);
          chatsJoined.add(chatName);
          saveListString('joinedChats', chatsJoined);
          chatsToNotify.add(chatName);
    ...
    IconButton enableDisableNotifications = new IconButton(
      icon: Icon(
        messageListCreated == false
            ? null
            : chatsToNotify.contains("chat_acc_"+id)
                ? Icons.volume_up
                : Icons.volume_off,
      ),
      onPressed: () async {
        if (chatsToNotify.contains("chat_acc_"+id)) {
          http.Response response = await http.post(
              getUrlHome() + "disableNotificationChat",
              headers: {HttpHeaders.authorizationHeader: await getToken()},
              body: {chatName: "chat_acc_"+id});
              checkResponseStatus(response, context);
          if (response.statusCode == 201) {
            unsubscribeChatNew(id, true);
            //showDialog (?)
            chatsToNotify.remove("chat_acc_"+id);
          ...
          if (response.statusCode == 201) {
            subscribeChatNew(id, true);
            chatsToNotify.add("chat_acc_"+id);
            await saveListString("chatsToNotify", chatsToNotify);
            setState(() {});
          } else {
            //showError
          ...
          showDialog(
              ...
              builder: (context) {
                return AlertDialog(
                  ...
                  content: Text(
                      "Hai lasciato la chat, clicca ok per tornare al menu delle chat.",style: TextStyle(
                color: Colors.white,
                  ...
                  builder: (context, size) {
                    ...
                    return new TextField(
                      ...
                      decoration: new InputDecoration.collapsed(
                          ...
                          hintStyle: TextStyle(color: Colors.white, fontFamily: 'Avenir Next')),
                    );
                  },
                )),
                new Container(
  ...
  void _createMessageList(
    ...
    for (int i = 0; i < jsonList.length; i++) {
      ...
      // print("J2");
      // print(jsonList[i]);
      //var jsonData = json.decode(jsonList[i]);
      print("Da2");
      //   print(jsonData);
